      ...
      {!isAddingItem && (
        <div className="flex flex-col space-y-4 h-full">
          ...
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-xl font-bold text-slate-100 tracking-tight">Catálogo</h2>
              <p className="text-xs text-slate-400">Serviços e estoque de peças</p>
            </div>
            <button
          ...
          <div className="flex bg-dark-900 border border-dark-800 rounded-lg p-0.75 gap-0.5">
            <button
              onClick={() => {
                setActiveSegment('services');
                setSearch('');
              }}
              className={`flex-1 py-1.5 rounded-md font-bold text-[10px] uppercase tracking-wider flex items-center justify-center gap-1.5 transition-all ${activeSegment === 'services' ? 'bg-brand-500 text-white shadow-premium' : 'text-dark-400 hover:text-slate-200'}`}
            >
              <Tag size={12} />
            ...
            <button
              ...
              className={`flex-1 py-1.5 rounded-md font-bold text-[10px] uppercase tracking-wider flex items-center justify-center gap-1.5 transition-all ${activeSegment === 'parts' ? 'bg-brand-500 text-white shadow-premium' : 'text-dark-400 hover:text-slate-200'}`}
            >
              <Package size={12} />
              Peças ({parts.length})
            </button>
          ...
          {/* Lists content */}
          <div className="flex-1 overflow-y-auto no-scrollbar pb-16 space-y-2.5">
            ...
            {activeSegment === 'services' && (
              filteredServices.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-center text-dark-500">
                  <h4 className="text-xs font-bold text-slate-400 mt-2">Nenhum serviço catalogado</h4>
                </div>
              ) : (
                filteredServices.map(item => (
                  <div key={item.id} className="bg-dark-900 border border-dark-800 rounded-xl p-3 flex items-start justify-between">
                    <div className="space-y-1 pr-4">
                      <div className="flex items-center gap-1.5 flex-wrap">
                        <span className="font-bold text-xs text-slate-200">{item.name}</span>
                        {item.code && (
                          <span className="text-[8px] font-mono bg-dark-950 border border-dark-800 text-dark-400 px-1.5 py-0.25 rounded uppercase">
                            {item.code}
                          </span>
                        )}
                      </div>
                      {item.description && (
                        <p className="text-[9px] text-dark-400 line-clamp-2 leading-snug">{item.description}</p>
                      )}
                    </div>
                    <div className="flex items-center gap-3 flex-shrink-0 text-right">
                      <div>
                        <div className="font-extrabold text-xs text-brand-400">{formatCurrency(item.price)}</div>
                        <span className="text-[8px] text-dark-500 font-bold uppercase">Mão de obra</span>
                      </div>
                      <div className="flex gap-1.5 items-center">
                        <button
                          onClick={() => handleStartEditService(item)}
                          className="p-1 hover:bg-dark-805 text-dark-400 hover:text-brand-400 rounded-lg transition-colors"
                        >
                          <Edit2 size={13} />
                        </button>
                        <button
            ...
            {activeSegment === 'parts' && (
              filteredParts.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-12 text-center text-dark-500">
                  <h4 className="text-xs font-bold text-slate-400 mt-2">Nenhuma peça no estoque</h4>
                </div>
              ) : (
                filteredParts.map(item => {
                  ...
                  return (
                    <div key={item.id} className="bg-dark-900 border border-dark-800 rounded-xl p-3 flex flex-col space-y-2">
                      <div className="flex justify-between items-start">
                        <div className="space-y-1">
                          ...
                          <div className="text-[9px] text-dark-400">
                            Forn: <span className="font-semibold text-dark-300">{item.supplier}</span>
                          </div>
                        </div>
                        
                        <div className={`text-[8px] font-bold px-2 py-0.5 border rounded-full flex items-center gap-1 ${stockColor}`}>
                          {item.stock <= 5 && item.stock > 0 && <AlertTriangle size={9} />}
                          {stockLabel}
                        </div>
                      </div>

                      <div className="flex items-center justify-between border-t border-dark-800/60 pt-2 text-[10px]">
                        <div className="flex gap-4">
                          <div>
                            <span className="text-[8px] text-dark-500 block">Compra:</span>
                            <span className="font-medium text-dark-300">{formatCurrency(item.purchasePrice)}</span>
                          </div>
                          <div>
                            <span className="text-[8px] text-brand-400 block font-bold">Venda:</span>
                            <span className="font-bold text-brand-400">{formatCurrency(item.salePrice)}</span>
                          </div>
                          <div>
                            <span className="text-[8px] text-success-500 block font-bold">Lucro:</span>
                            <span className="font-semibold text-success-500">{formatCurrency(item.salePrice - item.purchasePrice)}</span>
                          </div>
                        </div>
      ...
      {isAddingItem && (
        <div className="flex flex-col h-full bg-dark-950 animate-slide-in-right">
          ...
          <div className="flex items-center gap-2 border-b border-dark-800 pb-3 mb-4">
            <button onClick={handleCancel} className="p-1 hover:bg-dark-800 text-slate-300 rounded-lg transition-colors">
              <ArrowLeft size={16} />
            </button>
          ...
          {activeSegment === 'services' ? (
            /* Service Form */
            <form onSubmit={handleSubmitService} className="flex-1 overflow-y-auto no-scrollbar space-y-4 pb-20">
              <div className="space-y-1">
                <label className="text-[10px] uppercase font-bold text-dark-400">Nome do Serviço *</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-[10px] uppercase font-bold text-dark-400">Código / Referência</label>
                  <input
                    type="text"
                ...
                <div className="space-y-1">
                  <label className="text-[10px] uppercase font-bold text-dark-400">Valor Cobrado *</label>
                  <div className="relative">
              ...
              <div className="space-y-1">
                <label className="text-[10px] uppercase font-bold text-dark-400">Descrição do Serviço</label>
                <textarea
            ...
            /* Part Form */
            <form onSubmit={handleSubmitPart} className="flex-1 overflow-y-auto no-scrollbar space-y-4 pb-20">
              <div className="space-y-1">
                <label className="text-[10px] uppercase font-bold text-dark-400">Nome da Peça *</label>
                <input
                  type="text"
                  required
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-[10px] uppercase font-bold text-dark-400">Código SKU/Peça *</label>
                  <input
                    type="text"
                    required
                ...
                <div className="space-y-1">
                  <label className="text-[10px] uppercase font-bold text-dark-400">Estoque *</label>
                  <input
                    type="number"
                    required
                    placeholder="12"
              ...
              <div className="space-y-1">
                <label className="text-[10px] uppercase font-bold text-dark-400">Fornecedor</label>
                <input
                  type="text"
              ...
              <div className="grid grid-cols-2 gap-3">
                <div className="space-y-1">
                  <label className="text-[10px] uppercase font-bold text-dark-400">Valor de Compra</label>
                  <div className="relative">
                    <span className="absolute inset-y-0 left-0 pl-3 flex items-center text-dark-500 text-xs font-semibold">R$</span>
                    <input
                      type="number"
                ...
                <div className="space-y-1">
                  <label className="text-[10px] uppercase font-bold text-dark-400">Valor de Venda *</label>
                  <div className="relative">
                    <span className="absolute inset-y-0 left-0 pl-3 flex items-center text-dark-500 text-xs font-semibold">R$</span>
                    <input
                      type="number"
